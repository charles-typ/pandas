...
def pipeline_inner_join(const int64_t[:] left, const int64_t[:] right,
    ...
    with nogil:
        ...
        for i in range(len(right)):
            lc = right[i]
            if lc + 1 < left_length and left_count[lc + 1] > 0:
    ...
    with nogil:
        for i in range(len(right)):
            if right[i] + 1 < left_length and left_count[right[i] + 1] > 0:
...
@cython.boundscheck(False)
def pipeline_inner_merge_join(const int64_t[:] left, const int64_t[:] right,
               Py_ssize_t max_groups, leftsorter = None, leftcount = None):
    cdef:
        Py_ssize_t i, j, k, count = 0
        ndarray[int64_t] left_count, right_count, left_sorter, right_sorter
